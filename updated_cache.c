...
enum DIP_set_type_t assign_DIP_set_type (int set_number, int nsets)
{
  enum DIP_set_type_t DIP_set_type;
  int nsets_d32;
  nsets_d32 = nsets/32;
  if(set_number % nsets_d32 == 0)
    {
      DIP_set_type = LRU_type;
    }
  else if((set_number+1) % nsets_d32 == 0)
    {
      DIP_set_type = BIP_type;
    }
  else
    {
      DIP_set_type = Follower_type;
    }
  return DIP_set_type;
}

/* 
/* create and initialize a cache structure for L2 D cache */
struct cache_t *			/* pointer to cache created */
cache_create_dl2(char *name,		/* name of the cache */
	     int nsets,			/* total number of sets in cache */
	     int bsize,			/* block (line) size of cache */
	     int balloc,		/* allocate data space for blocks? */
	     int usize,			/* size of user data to alloc w/blks */
		  ...
		  */
{
  ...
  cp->BIPCTR_WIDTH = BIPCTR_WIDTH;
  cp->PSEL_WIDTH = PSEL_WIDTH;
  cp->PSEL = 0;
  cp->BIPCTR = 0;
  ...
  for (bindex=0,i=0; i<nsets; i++)
    {
      if(policy == DIP)
        cp->sets[i].DIP_set_type = assign_DIP_set_type(i, nsets); // NEED TO CONFIRM WORKING
      cp->sets[i].way_head = NULL;
      cp->sets[i].way_tail = NULL;
      /* get a hash table, if needed */
      if (cp->hsize)
	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  case DIP:
    ...
    enum list_loc_t locn;
    if(cp->sets[set].DIP_set_type == LRU_type)
      {
        locn = Tail;
        int PSEL_max = (1 << cp->PSEL_WIDTH) - 1;
        if(cp->PSEL < PSEL_max)
          cp->PSEL++;
      }
    else if(cp->sets[set].DIP_set_type == BIP_type)
      {
         if(cp->BIPCTR == 0)
           {
             locn = Head;
         ...
         int max_BIPCTR = (1 << cp->BIPCTR_WIDTH) - 1;
      ...
      {
        int PSEL_MSB = cp->PSEL >> (cp->PSEL_WIDTH - 1);
        if(PSEL_MSB == 1)
          {
            if(cp->BIPCTR == 0)
              locn = Head;
    ...
    update_way_list(&cp->sets[set], repl, locn);
    break;
  case FIFO:
    repl = cp->sets[set].way_tail;
