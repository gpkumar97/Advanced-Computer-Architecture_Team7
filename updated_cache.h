...
enum cache_policy {
  ...
  DIP,    /* replace a block based on dynamic insertion policy */
...
struct cache_t
{
  ...
  int BIPCTR;       /* BIP counter */
  int BIPCTR_WIDTH; /* BIP counter width */
  int PSEL;         /* PSEL counter */
  int PSEL_WIDTH;   /* PSEL counter width */
...
/* create and initialize a cache structure for the L2 D cache */
struct cache_t *			/* pointer to cache created */
